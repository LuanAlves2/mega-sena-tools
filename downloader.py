...
def download_and_process_lotofacil(
    url: str = "https://asloterias.com.br/download-todos-resultados-lotofacil",
) -> str:
    """Baixa e processa o histórico da Lotofácil."""
    ...
    if form:
        ...
        a = soup.find('a', attrs={'title': re.compile(r'Lotofacil.*Excel', re.I)})
        if a and a.get('href'):
            href = a['href']
            if href.startswith('javascript:'):
                m = re.search(r'document\.(\w+)\.submit', href)
    ...
    if 'spreadsheetml' not in ctype and 'ms-excel' not in ctype:
        alt_name = f"database/lotofacil_raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        with open(alt_name, 'wb') as f:
            f.write(rfile.content)
        logging.error(f"Arquivo não é Excel ({ctype}). Salvado em {alt_name}")
        print(f"Erro: provável HTML salvo em {alt_name}.")
        return alt_name

    raw_filename = f"database/lotofacil_raw_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
    ...
    out_name = "database/lotofacil_sorteios_passados.xlsx"
    df.to_excel(out_name, index=False)
    logging.info(f"Arquivo processado salvo como {out_name}")
    print(f"Download e processamento concluídos: {out_name}")
...
def download_and_process_quina(
    url: str = "https://asloterias.com.br/download-todos-resultados-quina",
) -> str:
    """Baixa e processa o histórico da Quina."""
    ...
    if form:
        ...
        a = soup.find('a', attrs={'title': re.compile(r'Quina.*Excel', re.I)})
        if a and a.get('href'):
            href = a['href']
            if href.startswith('javascript:'):
                m = re.search(r'document\.(\w+)\.submit', href)
    ...
    if 'spreadsheetml' not in ctype and 'ms-excel' not in ctype:
        alt_name = f"database/quina_raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        with open(alt_name, 'wb') as f:
            f.write(rfile.content)
        logging.error(f"Arquivo não é Excel ({ctype}). Salvado em {alt_name}")
        print(f"Erro: provável HTML salvo em {alt_name}.")
    ...
    raw_filename = f"database/quina_raw_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
    with open(raw_filename, 'wb') as f:
        f.write(rfile.content)
    logging.info(f"Excel bruto salvo como {raw_filename}")
    ...
    out_name = "database/quina_sorteios_passados.xlsx"
    df.to_excel(out_name, index=False)
    logging.info(f"Arquivo processado salvo como {out_name}")
    print(f"Download e processamento concluídos: {out_name}")
