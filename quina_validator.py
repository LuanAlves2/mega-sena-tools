...
from quina_analyzer import QuinaAnalyzer
from utils import (
    even_odd_count, count_primes, count_leq_40,
    count_mult_3, count_fib, count_ends0, count_dup_digits,
    diff_max_min
)


class QuinaValidator:
    """Realiza validações e geração de apostas para a Quina."""

    def __init__(self):
        self.analyzer = QuinaAnalyzer()
        self.pesos = self.analyzer.freq_dezenas

    def aposta_valida(self, nums):
        if len(set(nums)) != len(nums) or not all(1 <= n <= 80 for n in nums):
            return False
        k = len(nums)
        ...
        if not (self.analyzer.avg_sum_min * k <= s <= self.analyzer.avg_sum_max * k):
            return False
        if count_leq_40(nums) != round(self.analyzer.frac_le40 * k):
            return False
    ...
    def gerar_apostas(self, n_apostas, dezenas):
        ...
        while len(apostas) < n_apostas:
            cand = random.sample(range(1, 81), dezenas)
    ...
    def salvar_apostas(self, apostas, n_apostas, dezenas):
        ...
        fn = f"sorteios/Quina_{n_apostas}Apostas_{dezenas}Dezenas_{hoje}.xlsx"
