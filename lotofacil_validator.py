...
from lotofacil_analyzer import LotofacilAnalyzer
from utils import (
    even_odd_count, count_primes, count_leq_12,
    count_mult_3, count_fib, count_ends0, count_dup_digits,
    diff_max_min, quadrant_count
)


class LotofacilValidator:
    """Realiza validações e geração de apostas para a Lotofácil."""

    def __init__(self):
        self.analyzer = LotofacilAnalyzer()
        self.pesos = self.analyzer.freq_dezenas

    def aposta_valida(self, nums):
        if len(set(nums)) != len(nums) or not all(1 <= n <= 25 for n in nums):
            return False
        k = len(nums)
        ...
        if not (self.analyzer.avg_sum_min * k <= s <= self.analyzer.avg_sum_max * k):
            return False
        if count_leq_12(nums) != round(self.analyzer.frac_le12 * k):
            return False
        ...
        if diff_max_min(nums) != self.analyzer.modo_diff:
            return False
        if sum(1 for x in nums if 20 <= x <= 25) != self.analyzer.modo_cluster:
            return False
        if quadrant_count(nums) != self.analyzer.modo_quadrant:
            return False
        return True
    ...
    def gerar_apostas(self, n_apostas, dezenas):
        ...
        while len(apostas) < n_apostas:
            cand = random.sample(range(1, 26), dezenas)
    ...
    def salvar_apostas(self, apostas, n_apostas, dezenas):
        ...
        fn = f"sorteios/Lotofacil_{n_apostas}Apostas_{dezenas}Dezenas_{hoje}.xlsx"
